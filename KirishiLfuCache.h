...
#include "KirishiCachePolicy.h"

namespace KirishiCache
{

template<typename Key, typename Value> class KirishiLfuCache;
...
public:
    ...
    friend class KirishiLfuCache<Key, Value>;
};

template <typename Key, typename Value>
class KirishiLfuCache : public KirishiCachePolicy<Key, Value>
{
public:
    ...
    KirishiLfuCache(int capacity, int maxAverageNum = 1000000)
...
template<typename Key, typename Value>
void KirishiLfuCache<Key, Value>::getInternal(NodePtr node, Value& value)
{
...
template<typename Key, typename Value>
void KirishiLfuCache<Key, Value>::putInternal(Key key, Value value)
{   
    // 如果不在缓存中，则需要判断缓存是否已满
...
template<typename Key, typename Value>
void KirishiLfuCache<Key, Value>::kickOut()
{
...
template<typename Key, typename Value>
void KirishiLfuCache<Key, Value>::removeFromFreqList(NodePtr node)
{
    // 检查结点是否为空
    if (!node) 
        return;
...
template<typename Key, typename Value>
void KirishiLfuCache<Key, Value>::addToFreqList(NodePtr node)
{
    // 检查结点是否为空
    if (!node) 
        return;
...
template<typename Key, typename Value>
void KirishiLfuCache<Key, Value>::addFreqNum()
{
    curTotalNum_++;
    if (nodeMap_.empty())
        curAverageNum_ = 0;
...
template<typename Key, typename Value>
void KirishiLfuCache<Key, Value>::decreaseFreqNum(int num)
{
    // 减少平均访问频次和总访问频次
    curTotalNum_ -= num;
    if (nodeMap_.empty())
...
template<typename Key, typename Value>
void KirishiLfuCache<Key, Value>::handleOverMaxAverageNum()
{
    if (nodeMap_.empty())
        return;
...
template<typename Key, typename Value>
void KirishiLfuCache<Key, Value>::updateMinFreq() 
{
    minFreq_ = INT8_MAX;
...
template<typename Key, typename Value>
class KirishiHashLfuCache
{
public:
    KirishiHashLfuCache(size_t capacity, int sliceNum, int maxAverageNum = 10)
        ...
        , capacity_(capacity)
    {
        ...
        {
            lfuSliceCaches_.emplace_back(new KirishiLfuCache<Key, Value>(sliceSize, maxAverageNum));
        }
    }
...
private:
    ...
    std::vector<std::unique_ptr<KirishiLfuCache<Key, Value>>> lfuSliceCaches_; // 缓存lfu分片容器
};
